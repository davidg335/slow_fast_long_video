            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        if isinstance(module, BertLayerWithDoubleCrossAttention):
                            return module(
                                *inputs,
                                past_key_value,
                                output_attentions,
                                query_length,
                                apm_hidden_states=apm_hidden_states,
                                apm_attention_mask=apm_attention_mask,
                            )
                        else:
                            return module(
                                *inputs,
                ...
                if isinstance(layer_module, BertLayerWithDoubleCrossAttention):
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        # If a 2D or 3D attention mask is provided for the cross-attention
        # we need to make broadcastable to [batch_size, num_heads, seq_length, seq_length]
        if apm_hidden_states is not None:
            if type(apm_hidden_states) == list:
                apm_batch_size, apm_sequence_length, _ = apm_hidden_states[
                    0
                ].size()
            else:
                (
                    ...
                    apm_sequence_length,
                    _,
                ) = apm_hidden_states.size()
            apm_hidden_shape = (apm_batch_size, apm_sequence_length)

            if type(apm_attention_mask) == list:
                apm_extended_attention_mask = [
                    self.invert_attention_mask(mask) for mask in apm_attention_mask
                ]
            elif apm_attention_mask is None:
                apm_attention_mask = torch.ones(apm_hidden_shape, device=device)
                apm_extended_attention_mask = self.invert_attention_mask(
                    apm_attention_mask
                )
            else:
                apm_extended_attention_mask = self.invert_attention_mask(
            ...
            apm_extended_attention_mask = None
        ...
        encoder_outputs = self.encoder(
            ...
            apm_hidden_states=apm_hidden_states,
            apm_attention_mask=apm_extended_attention_mask,
        )
...
class MBBertSelfAttention(BertSelfAttention):
    def forward(
        ...
        # and values come from an encoder; the attention mask needs to be
        # such that the encoder's padding tokens are not attended to.
        is_cross_attention = encoder_hidden_states is not None #means is VMB layer
        is_apm_cross_attention=apm_hidden_states is not None
        if is_apm_cross_attention:
            key_layer = self.transpose_for_scores(self.key(apm_hidden_states))
            value_layer = self.transpose_for_scores(self.value(apm_hidden_states))
            attention_mask = apm_attention_mask
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        #print(f"Shape of attention_scores after initialized:{attention_scores.shape}")
        ...
        if attention_mask is not None and not is_cross_attention and not is_apm_cross_attention:
            # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
            if hasattr(self, 'query_memory_bank'):
        ...
        if (is_cross_attention or is_apm_cross_attention) and self.save_attention:
            self.save_attention_map(attention_probs)
            attention_probs.register_hook(self.save_attn_gradients)
        ...
        if not is_cross_attention and not is_apm_cross_attention:
            B, N, C = k.shape
...
class BertLayerWithDoubleCrossAttention(nn.Module):
    def __init__(self, original_layer, config):
        ...
        self.crossattention_apm = MBBertSelfAttention(config,is_cross_attention=True)        # Cross-Attention 2
