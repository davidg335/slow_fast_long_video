...
class VQATask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        for i, question_id in enumerate(samples["question_id"]):
            pred_answer_list = answers[i * self.num_beams : (i + 1) * self.num_beams]
            pred_qa_pairs.append({"question_id": question_id, "answer": pred_answer_list})
    ...
    def after_evaluation(self, val_result, split_name, epoch, dataset, **kwargs):
        ...
        if 'coco' in self.dataset_name:
            metrics = self._report_metrics_coco(result_file=result_file, split=split_name, dataset=dataset)
        else:
            metrics = self._report_metrics_vqa(result_file=result_file, split=split_name, dataset=dataset, dataset_name=self.dataset_name)
        return metrics

    @dist_utils.main_process
    def _report_metrics_vqa(self, result_file, split, dataset, dataset_name):
        annotation = dataset.annotation
        # class_labels = dataset.class_labels

        prediction_list = json.load(open(result_file, "r"))

        match_video_list = []
        for prediction in prediction_list:
            question_id = prediction['question_id']
            pred_answer_list = prediction['answer']
            if 'msvd' in dataset_name or 'msrvtt' in dataset_name:
                pred_answer_list = [pred_answer.split(' ')[0] for pred_answer in pred_answer_list]
            gt_answer = annotation[question_id]['answer']
            match_video = [1 if pred_answer == gt_answer else 0 for pred_answer in pred_answer_list]
    ...
    @dist_utils.main_process
    def _report_metrics_coco(self, result_file, split):
        """
